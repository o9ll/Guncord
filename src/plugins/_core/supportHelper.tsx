...
import { platformName } from "@guncordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Guncord developers"
        const guncordGroupIds = [
            "1020801845490356245"
        ];

        const isOriginalDev = originalDevIds.includes(userId);
        const isGuncordGroup = guncordGroupIds.includes(userId);

        if (!isOriginalDev && !isGuncordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Guncord developers";
