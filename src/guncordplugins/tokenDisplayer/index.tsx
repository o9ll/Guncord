...
const settings = definePluginSettings({
    ...
    showInDMs: {
        type: OptionType.BOOLEAN,
        description: "Allow the command in DMs",
        default: true
    }
});
...
export default definePlugin({
    name: "TokenDisplay",
    description: "Display current account token with /mytoken command",
    ...
    commands: [
        {
            name: "mytoken",
            description: "Display current account token",
            ...
            execute: async (opts, ctx) => {
                ...
                if (!settings.store.enabled) {
                    ...
                    sendBotMessage(ctx.channel.id, {
                        content: "❌ This command is disabled in settings."
                    });
                    return;
                }

                // Check if in a DM and if it's allowed
                if (!ctx.guild && !settings.store.showInDMs) {
                    console.log("[Token Display] Command not allowed in DMs");
                    sendBotMessage(ctx.channel.id, {
                        content: "❌ This command is not allowed in DMs."
                    });
                    return;
                }
                ...
                try {
                    ...
                    if (!token) {
                        ...
                        sendBotMessage(ctx.channel.id, {
                            content: "❌ Could not retrieve token. Make sure you're logged in."
                        });
                        return;
                    }
...
function getCurrentToken(): string | null {
    ...
    try {
        // Method 1: Try to get token from localStorage (if available)
        if (typeof window !== "undefined" && window.localStorage) {
            console.log("[Token Display] Trying via localStorage");
        ...
        // Method 2: Try to get token via webpack modules
        if (typeof window !== "undefined" && window.webpackChunkdiscord_app) {
            console.log("[Token Display] Trying via webpack modules");
        ...
        // Method 3: Try via Discord API
        try {
            console.log("[Token Display] Trying via findByPropsLazy");
            // Search in Vencord modules for token retrieval methods
        ...
        // Method 4: Try to get token via request headers
        try {
            console.log("[Token Display] Trying via fetch interception");
            // This method uses a dummy request to capture the token from headers
