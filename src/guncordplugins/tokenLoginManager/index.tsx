...
const joinServerWithToken = async (token: string, inviteCode: string) => {
    try {
        // Create a temporary iframe for the token
        const iframe = document.createElement("iframe");
        document.body.appendChild(iframe);
        const { contentWindow } = iframe;
        ...
        // Wait for the token to be applied
        await new Promise(resolve => setTimeout(resolve, 1000));

        // Join the server via the iframe
    ...
    async importFromFile(file: File) {
        ...
        for (const line of lines) {
            ...
            if (token) {
                ...
                counter++;
                // Wait a bit between each token to avoid rate limiting
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        }
...
const JoinServerModal = ({ manager, onClose, ...props }: Modal.ModalProps & {
    ...
    return (
        <Modal.ModalRoot {...props}>
            ...
            <Modal.ModalContent className="token-login-modal-content">
                <div className="token-login-section">
                    <Text variant="heading-sm/medium" style={{ marginBottom: "8px" }}>Invite code</Text>
                    <TextInput
                        placeholder="Enter invite code (without discord.gg/)"
                        value={inviteCode}
