...
const logger = new Logger("VoiceHighQuality");
...
export default definePlugin({
    ...
    getAudioBitrate() {
        ...
        const boosted = settings.store.prioritizeAudioQuality
            ...
            : bitrate;
        logger.info(`Audio Bitrate: ${boosted} kbps`);
        return boosted;
    },
    ...
    start() {
        ...
        logger.info(`Bitrate: ${settings.store.audioBitrate} kbps`);
        logger.info(`Krisp: ${settings.store.enableKrispNoiseSuppression ? "ON" : "OFF"}`);
        logger.info(`Echo Cancel: ${settings.store.enableEchoCancellation ? "ON" : "OFF"}`);
        logger.info(`AGC: ${settings.store.enableAutoGainControl ? "ON" : "OFF"}`);
        logger.info(`Prioritize Quality: ${settings.store.prioritizeAudioQuality ? "ON" : "OFF"}`);
        logger.info("Maximum quality mode enabled");
    },
