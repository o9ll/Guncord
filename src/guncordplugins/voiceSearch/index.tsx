...
import { ModalCloseButton, ModalContent, ModalHeader, ModalRoot, openModal } from "@utils/gunModals";
import definePlugin from "@utils/types";
import { findByPropsLazy } from "@webpack";
import { ChannelStore, Forms, GuildChannelStore, GuildStore, IconUtils, PermissionsBits, PermissionStore, React, UserStore, useEffect, useMemo, useRef, useState, VoiceStateStore } from "@webpack/common";
import { Logger } from "@utils/Logger";

import sidebarStyle from "./sidebar.css?managed";

const logger = new Logger("VoiceSearch");
                ...
                for (const guildId in guilds) {
                    ...
                    for (const key of Object.keys(allChannels)) {
                        ...
                        for (const item of arr) {
                            ...
                            const channel = ChannelStore.getChannel(ch.id) ?? ch;
                            out.push({
                                channelId: ch.id,
                                channelName: cName,
                                ...
                                canAccess: channel ? PermissionStore.can(PermissionsBits.VIEW_CHANNEL, channel) && PermissionStore.can(PermissionsBits.CONNECT, channel) : false,
...
export default definePlugin({
    ...
    start() {
        ...
        addHeaderBarButton("gun-voice-channel-search", () => <VCSHeaderButton />, 9);
    },
    stop() {
        disableStyle(sidebarStyle);
        removeHeaderBarButton("gun-voice-channel-search");
        scanCache = null;
    },
});
