...
import { getGroqKey, groqChat } from "../guncordAI/groqManager";
            ...
            <div style={{
                ...
                <div>
                    ...
                    <div style={{ fontSize: "13px", marginTop: "4px" }}>
                        ...
                        Please configure it once in the <strong>{t("GuncordAI")}</strong>{t("settings.")}</div>
                </div>
            </div>
        )
    },
        ...
        if (!apiKey) {
            try {
                ...
                openConfirmationModal({
                    ...
                    content: "AutoResponder requires a Groq API Key to function. Please configure it once in the GuncordAI settings.",
                    confirmText: "Configure GuncordAI",
                    cancelText: "Cancel",
                    onConfirm: () => {
                        const { openModal } = findByPropsLazy("openModal");
                        // Logic to open GuncordAI settings if possible
                    }
                });
            } catch (e) {
                                ...
                                if (!key) {
                                    openModal(props2 => (
                                        <ModalRoot {...props2} size={ModalSize.SMALL}>
                                            ...
                                            <ModalContent>
                                                <Text variant="text-md/normal" style={{ marginBottom: 16 }}>
                                                    {t("AutoResponder requires a Groq API Key to function. Please configure it once in the GuncordAI settings.")}
                                                </Text>
                                            </ModalContent>
                                            <div style={{ padding: "16px", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
