 ...
 */
export function initGuncordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Guncord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "guncord", "Uninstall Guncord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Guncord", "Uninstall Guncord.exe"),
                            join(process.env.PROGRAMFILES || "", "Guncord", "Uninstall Guncord.exe"),
                        ];
            ...
            console.log("[Guncord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Guncord] Failed to create tray:", e);
        }
    });
}
