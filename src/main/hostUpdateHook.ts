...
/**
 * re-applies the guncord patch to a freshly installed discord host
...
interface DiscordUpdaterModule {
    ...
    __guncordTryInitWrapped?: boolean;
}
...
interface DiscordDesktopCore {
    ...
    __guncordStartupWrapped?: boolean;
}

const error = (...args: unknown[]) => console.error("[Guncord:HostUpdate]", ...args);
...
const retainGuncord = (updater: DiscordHostUpdater, reason: string) => {
    try {
...
const attachToUpdater = (updater: DiscordHostUpdater | null | undefined) => {
    ...
    updater.on?.("host-updated", () => retainGuncord(updater, "host-updated"));
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options?: StartCurrentVersionOptions) => {
            bound(options);
            try { retainGuncord(updater, "startCurrentVersionSync"); } catch (e) { error(e); }
        };
    }
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions?: object, options?: StartCurrentVersionOptions) => {
            ...
            try { retainGuncord(updater, "startCurrentVersion"); } catch (e) { error(e); }
        };
    }
};

const wrapStartup = (coreExports: DiscordDesktopCore | null | undefined) => {
    if (!coreExports?.startup || coreExports.__guncordStartupWrapped) return;
    coreExports.__guncordStartupWrapped = true;
    ...
    coreExports.startup = function (opts, ...rest) {
        try {
            ...
            } else if (typeof updaterModule?.tryInitUpdater === "function" && !updaterModule.__guncordTryInitWrapped) {
                /*
                 * updater not yet constructed at startup time. wrap the
                 * factory so we attach once vanilla creates it.
                 */
                updaterModule.__guncordTryInitWrapped = true;
