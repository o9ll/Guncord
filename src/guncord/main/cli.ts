...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.GuncordDir) {
        console.error("Cannot repair: using custom Guncord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Guncord...");
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Guncord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Guncord v${app.getVersion()}
