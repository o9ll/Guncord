...
handleSync(IpcEvents.GET_GIT_HASH, () => Guncord_GIT_HASH);
...
// Handler for VencordNative.guncord.relaunch() — used by Restart buttons in plugin settings
handle(IpcEvents.RELAUNCH_APP, async () => {
    ...
    // On packaged Windows (NSIS/Squirrel), app.relaunch() may silently fail
    // because process.execPath points to the internal Electron binary, not the launcher.
    // Use spawn to directly relaunch the main executable.
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.GuncordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { GuncordDir } = State.store;
    if (!GuncordDir) return;

    const stats = await stat(GuncordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(GuncordDir);
});
