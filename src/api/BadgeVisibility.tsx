...
export type BadgeSource = "vencord" | "equicord" | "guncord" | "globalbadges";
...
export async function loadOwnHiddenBadgeSources(userId: string) {
    ...
    try {
        const localData = localStorage.getItem("guncord_hidden_badges");
        if (localData) {
    ...
    try {
        ...
        if (result?.config?.settings?.hidden !== undefined) {
            ...
            localStorage.setItem("guncord_hidden_badges", JSON.stringify(hidden));
        }
    } catch (e) {
