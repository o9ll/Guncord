...
function syncDiscordLocale(lang: Language) {
    try {
        ...
        if (UserSettingsProtoUtils?.updateAsync) {
            Promise.resolve(UserSettingsProtoUtils.updateAsync("localization", (pref: any) => {
                ...
                Promise.resolve(UserSettingsProtoUtils.updateAsync("locale", (pref: any) => {
                    if (pref) pref.locale = { value: targetLocale };
                }, 0)).catch(e => console.error("[Guncord] Failed to update locale proto:", e));
            });
        }
        ...
        if (api && typeof api.patch === "function") {
            ...
            }).catch((e: any) => console.error("[Guncord] Failed to PATCH settings:", e));
        }
        ...
        console.error("[Guncord] Failed to sync Discord language:", e);
    }
}
...
function LanguageTab() {
    ...
    function selectLang(lang: Language) {
        ...
        Alerts.show({
            ...
            body: t("Guncord language has been changed. Discord must be restarted for all translations to take effect. Restart now?"),
    ...
    return (
        <SettingsTab>
            ...
            <Paragraph className={Margins.bottom16}>
                {t("Choose the language for Guncord's interface. Plugin names and Discord's own UI are not affected.")}
            </Paragraph>

            <Notice.Info className={Margins.bottom20}>
                {t("Translations are community-maintained and may be incomplete. If you'd like to help translate Guncord, contributions are welcome!")}
            </Notice.Info>
            ...
            <div className={Margins.top16}>
                <FormSwitch
                    ...
                    description={t("Automatically change Discord's client language when changing Guncord language.")}
