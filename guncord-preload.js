// Guncord preload — globalPaths fix + Equicord with contextBridge
"use strict";
    ...
    try {
        ...
        if (rendererJs) {
            webFrame.executeJavaScript(rendererJs).catch(e => {
                console.error("[Guncord] renderer inject failed:", e?.message);
            });
        }
    } catch (e) {
        console.error("[Guncord] VencordPreloadGetRendererJs failed:", e);
    }
