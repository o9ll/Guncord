...
async function shouldAutoRestart() {
    try {
        const prefsPath = path.join(process.env.APPDATA, "Guncord", "settings", "installer-prefs.json");
...
async function deleteShims(paths) {
    ...
    for (const resPath of paths) {
        log(`Removing Guncord from: ${resPath}`);
        try {
            ...
            const dirsToClean = ["mac", "multi-instance-icons", "server"];
            ...
            log(`❌ Could not remove Guncord from ${resPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(paths) {
    try {
        ...
        lognewline("Deleting Guncord loader and restoring files...");
