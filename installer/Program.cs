...
namespace GuncordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private GuncordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Guncord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GuncordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "GuncordInstaller_WebView2");
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show(
                        "Microsoft Edge WebView2 Runtime is required to run the Guncord Installer but was not found on your system.\n\n" +
                        "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                        "After installing, restart the Guncord Installer.",
            ...
            _backend = new GuncordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GuncordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GuncordInstaller.icon.ico"))
                    {
    ...
    public class GuncordBackend
    {
        ...
        const string GITHUB_REPO = "o9ll/Guncord";
        const string GITHUB_URL  = "https://api.github.com/repos";
        const string DIST_ZIP   = "guncord-dist.zip";

        public GuncordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Guncord", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"guncord\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Guncord is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Guncord → not a third-party mod
                if (content.Contains("\"guncord\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "Guncord-Installer/2.0");
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "guncord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Guncord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            SetProgress(92, "Configuring Guncord loader...");
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            {
                Console.WriteLine($"[Guncord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"guncord\""))
                {
            ...
            string[] dirsToClean = { "mac", "multi-instance-icons", "server" };
        ...
        private void WriteLoader(string appDir)
        {
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"guncord\",\"main\":\"index.js\"}");
        ...
        private void CopyAssetsToDiscord(string resPath)
        {
            ...
            string[] dirsToCopy = { "mac", "multi-instance-icons", "modules", "server" };
