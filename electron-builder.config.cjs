...
// ─── Guncord Build Configuration ─────────────────────────────────────────

function killGuncord() {
    const releaseDir = join(__dirname, "release", "guncord-dist");
...
function buildGuncordFromDiscord(discordApp) {
    ...
    console.log("[guncord] Preparing _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Guncord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "guncord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const guncordPreload = join(__dirname, "guncord-preload.js");
    if (existsSync(guncordPreload)) {
    ...
    if (existsSync(macCursorsSrc)) {
        console.log("[guncord] Copying macOS cursors...");
        cpSync(macCursorsSrc, join(outRes, "mac"), { recursive: true });
    } else {
        console.warn("[guncord] WARNING: 'mac' folder not found at root, CursorMacOS plugin will not work in the packaged build.");
    }
    ...
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Guncord" --set-version-string "FileDescription" "Guncord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[guncord] Build finished -> ${outDir}`);
}
...
buildGuncordFromDiscord(discord);

module.exports = {
    appId: "com.guncord.app",
    productName: "Guncord",
    copyright: "Copyright 2026 Guncord",
