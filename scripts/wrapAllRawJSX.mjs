...
const targetPlugins = [
    "FakeVoice", "antiMoveDeco", "guncordAI", "ClearGroups", "ClearDMs", "messageCleaner",
...
function isRealText(str) {
    ...
    if (str === "Guncord" || str === "Guncord AI") return false;
...
for (const pluginFolder of targetPlugins) {
    const pluginPath = join("src/guncordplugins", pluginFolder);
    ...
    for (const file of files) {
        ...
        if (validRawJSX.length > 0) {
            // Check import
            if (!content.includes('autoTranslateGuncord')) {
                // Find relative path to autoTranslateGuncord
                const depth = file.split(/[/\\]/).length - file.split(/[/\\]/).indexOf("guncordplugins") - 2;
                const relPath = "../".repeat(Math.max(1, depth)) + "autoTranslateGuncord";
