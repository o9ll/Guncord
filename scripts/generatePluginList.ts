...
(async () => {
    ...
    // Setup userplugins
    try {
        const userTarget = join(homedir(), "Documents", "Guncord", "userplugins");
        ...
        console.error("[Guncord] Failed to setup userplugins link", e);
    }
    ...
    const guncordFlag = args.includes("--guncord");

    let dirs: string[];

    if (guncordFlag) {
        dirs = ["src/guncordplugins"];
    } else if (equicordFlag) {
        dirs = ["src/guncordplugins", "src/userplugins"];
    } else if (vencordFlag) {
        dirs = ["src/plugins", "src/plugins/_core"];
    } else {
        dirs = ["src/plugins", "src/plugins/_core", "src/guncordplugins", "src/userplugins"];
    }
