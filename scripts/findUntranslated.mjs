...
for (const pluginFolder of targetPlugins) {
    const pluginDir = join("src/guncordplugins", pluginFolder);
    let files = [];
    try {
    ...
    for (const file of files) {
        ...
        const rawJSX = [...content.matchAll(/>\s*([A-Za-z][^<>{}\n]*[A-Za-z0-9!?.:])\s*</g)]
            .map(m => m[1].trim())
            .filter(str => str !== "Guncord" && str !== "Guncord AI" && !str.includes("https://"));
        ...
        if (missingInDict.length > 0) {
            console.log(`  [${file}] t() keys missing from autoTranslateGuncord (${missingInDict.length}):`, missingInDict);
