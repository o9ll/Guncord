...
const pluginsDir = path.join(process.cwd(), 'src', 'guncordplugins');
...
for (const p of pluginDirs) {
    ...
    if (nameMatch && descMatch) {
        ...
        // Add some specific instructions based on common Guncord plugin features
...
// Also let's handle Vencord plugins by just providing a fallback in the component, but we can do it here for guncord plugins.
