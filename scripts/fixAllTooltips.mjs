...
const files = getFiles("src/guncordplugins");
const tooltipsFound = [];
...
for (const file of files) {
    ...
    for (const m of tooltipMatches) {
        const attr = m[1];
        const val = m[2].trim();
        if (val && val !== "Guncord" && val !== "Guncord AI" && !val.startsWith("http") && !/^\d+$/.test(val)) {
            tooltipsFound.push({ file, attr, val });
            
            // Check if file has t imported
            if (!content.includes('autoTranslateGuncord') && !content.includes('import { t }') && !content.includes('import { tPlugin as t }')) {
