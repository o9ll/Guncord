...
// Read current autoTranslateGuncord index.ts
const atcContent = readFileSync("src/guncordplugins/autoTranslateGuncord/index.ts", "utf8");

// Parse existing entries in autoTranslateGuncord
...
console.log(`Existing entries in autoTranslateGuncord: ${Object.keys(existingEntries).length}`);

// Find strings that are not in existingEntries or have missing/corrupt translations
const missingStrings = strings.filter(s => {
    if (s === "Guncord" || s === "Guncord AI") return false;
...
async function main() {
    ...
    // Format new autoTranslateGuncord index.ts
    let newContent = `/*
...
export default definePlugin({
    name: "AutoTranslateGuncord",
    enabledByDefault: true,
    required: true,
    description: "Automatic translation for Guncord.",
...
`;
    ...
    writeFileSync("src/guncordplugins/autoTranslateGuncord/index.ts", newContent, "utf8");
