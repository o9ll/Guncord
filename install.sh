...
GITHUB_API="https://api.github.com/repos/o9ll/Guncord"
...
download_guncord_bundle() {
    ...
    TAG="$(curl -sSL "$GITHUB_API/releases/latest" 2>/dev/null | grep -o '"tag_name": *"[^"]*"' | cut -d'"' -f4 || echo "latest")"
    if [ -z "$TAG" ]; then TAG="latest"; fi
    log_info "Detected latest version: ${WHITE}$TAG${RESET}"

    DOWNLOAD_URL="https://github.com/o9ll/Guncord/releases/download/$TAG"

    # 1. Try downloading guncord-dist.zip from the release
...
install_macos() {
    ...
    if [ ${#FOUND_PATHS[@]} -gt 1 ]; then
        echo -e "  ${CYAN}[$(( ${#FOUND_PATHS[@]} + 1 ))]${RESET} ${GREEN}All installations${RESET}"
    fi
...
install_linux() {
    ...
    if [ ${#FOUND_PATHS[@]} -gt 1 ]; then
        echo -e "  ${CYAN}[$(( ${#FOUND_PATHS[@]} + 1 ))]${RESET} ${GREEN}All detected installations${RESET}"
    fi
